...
export function Navbar({ onScrollToTop, tableNumber, sessionId }: NavbarProps) {
  ...
  const handleLogoClick = () => {
    if (isHome) {
      // Se siamo già sulla home, scroll top (comportamento originale)
      window.scrollTo({ top: 0, behavior: "smooth" });
      onScrollToTop?.();
    } else {
      // Su qualsiasi altra pagina, torna alla home
      router.push("/");
    }
  };
      ...
      <div className="max-w-7xl mx-auto px-6 h-20 flex justify-between items-center">
        ...
        <nav className="hidden md:flex items-center gap-1 bg-gray-50/50 p-1 rounded-full border border-gray-100">
          {tableNumber && sessionId ? (
            <>
              <span className="px-5 py-2 text-sm font-semibold text-gray-500 tracking-widest uppercase">
                Tavolo {tableNumber}
              </span>
              <Link
